...
client.on('ready', function(){
    var ms = 60000 ;
    var setGame = ['@help','VICTORS SERVER'];
    var i = -1;
    var j = 0;
 ...
 client.on('message', message => {
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .setColor('RANDOM')
            .setTitle('``INFO Ghaleb Community ©`` ')
...
client.on('message', message => {
    ...
    translate(finalToTrans, {to: language}).then(res => {
            message.channel.send({embed: {
                color: 3447003,
                author: {
                  name: 'Ghaleb Community ©\'s translator',
...
client.on('message', message => {
    if (message.content.startsWith(prefix + 'GBans')) {
        message.guild.fetchBans()
        .then(bans => message.channel.send(`Number of banned persons **${bans.size}** `))
}
});
client.on('message', message => {
    if (message.content.startsWith(prefix + 'Gavater')) {
if(!message.channel.guild) return;
...
message.channel.send(
  ...
  .setColor("#9B59B6")
  .addField(" ** :gear: Server Support :gear: **" , "  **https://discord.gg/jEbzskC**")
...
client.users.get("411603069516906517").send(
    "\n" + "**" + "? السيرفر :" + "**" +
    "\n" + "**" + "» " + message.guild.name + "**" +
    "\n" + "**" + " ? المرسل : " + "**" +
    "\n" + "**" + "» " + message.author.tag + "**" +
...
.setDescription(`برودكاست بـ امبد NE\nبرودكاست بدون امبدE \nلديك دقيقه للأختيار قبل الغاء البرودكاست`)
if (!args) return message.reply('**يجب عليك كتابة كلمة او ??ملة لإرسال البرودكاست**');message.channel.send(BcList).then(msg => {
msg.react('??')
.then(() => msg.react('?'))
.then(() =>msg.react('??'))
 
let EmbedBcFilter = (reaction, user) => reaction.emoji.name === 'E' && user.id === message.author.id;
let NormalBcFilter = (reaction, user) => reaction.emoji.name === 'NE' && user.id === message.author.id;
...
client.on('guildCreate', guild => {
  ...
  .setDescription(`**
  New Server Add Ghaleb Community © ?
...
client.on("message", message => {
 if (message.content === `${prefix}`) {
  const embed = new Discord.RichEmbed() //by ,$ Ghaleb4777 'CODES SERVER'
      .setColor("#000000")//by ,$ Ghaleb4777 'CODES SERVER'
      .setDescription(`
${prefix}play ? لتشغيل أغنية برآبط أو بأسم
${prefix}skip ? لتجآوز الأغنية الحآلية
${prefix}pause ? إيقآف الأغنية مؤقتا
...
${prefix}np ? لمعرفة الأغنية المشغلة حآليا
${prefix}queue ? لمعرفة قآئمة التشغيل
 `)//by ,$ Ghaleb4777 'CODES SERVER'
   message.channel.sendEmbed(embed)//by ,$ Ghaleb4777 'CODES SERVER'
...
client.login("NTg4MzIzNDI3NjE0NzIwMDAx.XQDc6Q.Dc09Gal16NqYJoyzcPaUdH3qBbI")
